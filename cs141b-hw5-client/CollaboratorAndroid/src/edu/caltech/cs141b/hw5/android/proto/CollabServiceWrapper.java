...
/**
 * Implements the API available from CollaboratorServiceImpl while hiding away
 * the details of protocol buffer and HTTP connection.
 * 
 * @author aliu
 * 
 */
public class CollabServiceWrapper {
	...
	public List<DocumentMetadata> getDocumentList() {
		...
		RequestMessage request = RequestMessage.newBuilder()
				.setRequestType(RequestType.GET_DOCUMENT_LIST).build();
		...
		try {
			...
			Log.e(TAG,
					"Getting response terminated from exception."
							+ e.getMessage());
		}
		return docs;
	}
	 ...
	 */
	public LockedDocument lockDocument(String key) throws LockUnavailable,
			InvalidRequest {

		RequestMessage request = RequestMessage.newBuilder()
				.setRequestType(RequestType.LOCK_DOCUMENT).setDocumentKey(key)
				.build();
		...
		try {
			...
			case SUCCESS:
				return DataConverter.buildLockedDocument(response
						.getLockedDoc());
	...
	public UnlockedDocument getDocument(String key) throws InvalidRequest {
		RequestMessage request = RequestMessage.newBuilder()
				.setRequestType(RequestType.GET_DOCUMENT).setDocumentKey(key)
				.build();
		...
		try {
			...
			switch (response.getStatusType()) {
			case SUCCESS:
				return DataConverter.buildUnlockedDocument(response
						.getUnlockedDoc());
			default:
				throw new InvalidRequest(response.getMessage());
			}
		...
		try {
			...
			case SUCCESS:
				return DataConverter.buildUnlockedDocument(response
						.getUnlockedDoc());
			case LOCK_EXPIRED:
				Log.e(TAG, "LOCK EXPIREEEED IN WRAPPER");
				throw new LockExpired(response.getMessage());
			default:
				Log.e(TAG, "Invalid request in wrapper!");
				throw new InvalidRequest(response.getMessage());
			}
		} catch (InterruptedException e) {
			Log.e(TAG, "Thrad interrupted." + e.getMessage());
